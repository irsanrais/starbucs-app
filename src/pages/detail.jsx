...
export default function DetailProduct() {
  ...
  const countCart = async () => {
    try {
      let count = await axios.get(`http://localhost:5000/carts`);
      console.log(count);
      setDataCart(count.data.length);
    } catch (error) {}
  };
    ...
    <div>
      ...
      <div className="flex items-center bg-green-800" style={{ height: "300px" }}>
        ...
        <div className="basis-3/5 text-white">
          ...
          <h1 className="basis-3/5 text-2xl mt-2">{data.size[0].calories} Calories</h1>
          <h1 className="basis-3/5 text-2xl mt-2">Rp. {data.size[0].price.toLocaleString()}</h1>
        </div>
      </div>
